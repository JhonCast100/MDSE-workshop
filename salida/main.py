...
def put_facturas_factura_id(factura_id: int, data: Factura):
    """Actualizar un factura existente"""
    if factura_id >= len(facturas_db):
        raise HTTPException(status_code=404, detail="Factura no encontrado")
    facturas_db[factura_id] = data
    return data


@app.delete("/facturas/{factura_id}", response_model=dict)
def delete_facturas_factura_id(factura_id: int):
    """Eliminar un factura"""
    if factura_id >= len(facturas_db):
        raise HTTPException(status_code=404, detail="Factura no encontrado")
    facturas_db.pop(factura_id)
